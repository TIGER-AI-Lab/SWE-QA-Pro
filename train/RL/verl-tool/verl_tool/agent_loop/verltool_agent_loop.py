...
class VerlToolAgentLoop(AgentLoopBase):
    ...
    async def run(self, sampling_params: dict[str, Any], **kwargs) -> AgentLoopOutput:
        ...
        for step in range(max_turns + 1):
            ...
            force_prompt_ids = [] 
            if is_last_step and self.agent_config.force_finish_for_last_turn and use_tool:
                force_prompt = (
                    f"--- MAX TURNS REACHED ---\n"
                    "This is the final allowed turn. You MUST respond using plain text without calling any tools. Don't generate any <tool_call> ... </tool_call>"
                    "Please provide your best possible final answer now following OUTPUT PROTOCOL."
                    "Wrap your entire final answer inside a <finish>...</finish> block."
                )
                force_prompt_ids = self.tokenizer.encode(
                    force_prompt,
                    add_special_tokens=False,
                )
                running_prompt_ids.extend(force_prompt_ids)
                response_mask.extend([0] * len(force_prompt_ids))
                response_logprobs.extend([0.0] * len(force_prompt_ids))
                
            max_tokens_for_this_turn = max(1, max_tokens_for_this_turn - len(force_prompt_ids))
