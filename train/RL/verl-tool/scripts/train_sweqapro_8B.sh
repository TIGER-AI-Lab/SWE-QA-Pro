set -x
train_data=[$(pwd)/dataset/train.parquet]
val_data=[$(pwd)/dataset/test.parquet]
...
export WANDB_PROJECT="SWE-QA-Pro-test"
export WANDB_START_METHOD=thread
export WANDB__SERVICE_WAIT=300
STAGE1_RUN_NAME="SWE-QA-Pro_8B_RL"
export WANDB_NAME=$STAGE1_RUN_NAME
...
python -m verl_tool.servers.serve --host $host --port $port --tool_type "swe_qa_pro" --workers_per_tool 8 &
server_pid=$!

echo "Server (pid=$server_pid) started at $tool_server_url"

# OPENAI_API_KEY / OPENAI_BASE_URL are read from RL/verl-tool/.env by the
# sweqapro reward manager (the judge). See .env.example.
